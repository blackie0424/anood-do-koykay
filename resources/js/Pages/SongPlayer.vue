...
const displayMode = ref('both') // 'native' | 'zh' | 'both'
...
const modeLabel = computed(() => {
  return { both: '全部', native: '族語', zh: '中文' }[displayMode.value]
})
</script>
  ...
  <div class="min-h-screen bg-stone-50 p-4 pb-32">
    <div class="max-w-2xl mx-auto">
      ...
      <div class="space-y-2">
        <div
          ...
          @click="playLine(line)"
          :class="[
            'rounded-xl p-4 transition-colors cursor-pointer select-none',
            idx === activeLineIndex
        ...
        >
          <p
            ...
            style="font-size: clamp(1.25rem, 4vw, 1.5rem)"
          >
            {{ line.text_native }}
          </p>
          <p
            v-if="displayMode !== 'native'"
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 p-4">
      <div class="max-w-2xl mx-auto flex items-center gap-4">
        <button
          @click="togglePlay"
          :disabled="!song.audio_full || hasError"
          :class="[
            'flex-shrink-0 w-16 h-16 rounded-full text-2xl flex items-center justify-center transition-transform active:scale-95',
            song.audio_full && !hasError
