...
async function copyLink(song) {
    await navigator.clipboard.writeText('https://anood.pongsonotao.org/songs/' + song.id)
            ...
            <div class="max-w-2xl mx-auto space-y-4">
                <div v-for="song in filteredSongs" :key="song.id"
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0 ml-4">
                        <a v-if="isMobile && !isPwa" :href="lineShareUrl(song.id)" target="_blank" rel="noopener"
                            aria-label="分享到 LINE"
                        ...
                        <button v-else @click="copyLink(song)"
