...
function saveSong() {
  if (props.song?.id) {
    songForm.put(`/admin/songs/${props.song.id}`)
  } else {
    songForm.post('/admin/songs')
  }
}
...
async function uploadAudio(e) {
  ...
  fd.append('type', 'full')
  try {
    await axios.post(`/api/admin/songs/${props.song.id}/audio`, fd)
  } catch {
...
async function saveLines() {
  ...
  try {
    await axios.post(`/api/admin/songs/${props.song.id}/lines/batch`, { lines: lines.value })
  } finally {
  ...
  <div class="p-6 max-w-3xl mx-auto space-y-8">
    ...
    <section class="bg-white rounded-lg shadow p-6 space-y-4">
      ...
      <div>
        ...
        <input v-model="songForm.title_native" type="text" required
          class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>
      <div>
        <label class="block text-sm font-medium mb-1">中文名稱</label>
        <input v-model="songForm.title_zh" type="text"
          class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>
      <div>
      ...
      <button @click="saveSong" :disabled="songForm.processing"
    ...
    <template v-if="song?.id">
      ...
      <section class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div class="space-y-3">
          <div v-for="(line, idx) in lines" :key="idx" class="border rounded p-3 space-y-2">
            <div class="flex gap-2 items-center">
              <span class="text-stone-400 text-sm w-6">{{ idx + 1 }}</span>
              <input v-model="line.text_native" placeholder="族語歌詞"
                class="flex-1 border rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400" />
              <input v-model="line.text_zh" placeholder="中文翻譯"
                class="flex-1 border rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400" />
              <input v-model.number="line.start_time" type="number" step="0.1" placeholder="起始(秒)"
                class="w-24 border rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400" />
        ...
        <button @click="saveLines" :disabled="savingLines"
